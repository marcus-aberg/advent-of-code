        ...
        for j, loc in enumerate(row):
            ...
            for s in surroundings:
                ...
                if (
                    ...
                    if (
                        height(data[sr][sc]) - height(loc)
                    ) <= 1:  # Allowed step - TODO: I probably want to prioritize search directions that lead upward
...
def find_shortest_path(S, E, data):
    ...
    while True:
        ...
        if (
            current["id"] == E
        ):  # Goal: When E is on top of your priority queue and E is "popped" ->  this is when we have finished the algorithm
        ...
        for child in nodes[current["id"]]["children"]:
            ...
            if (
                len([v for v in visited if v["id"] == child]) != 0
            ):  # If we already visited this node - TODO: Improve this to not have to search every time
            ...
            if child_item is None:  # Unseen node
                ...
                queue_item = {
                    ...
                    "length": child_length,
                    "prior": current,
                }  # TODO: Maybe I can add some heuristic to improve the search direction
        ...
        priority_queue.sort(
            key=sort_criteria
        )  # TODO: Maybe I can insert the nodes in the priority-queue properly instead of sorting it every time
